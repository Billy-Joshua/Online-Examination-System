...
class JurusanMatkul extends CI_Controller {
	...
	public function save()
	{
		...
		if($this->form_validation->run() == FALSE){
			...
			if($method==='add'){
				$action = $this->master->create('department_courses', $input, true);
			}else if($method==='edit'){
				$id = $this->input->post('matkul_id', true);
				$this->master->delete('department_courses', $id, 'course_id');
				$action = $this->master->create('department_courses', $input, true);
			}
	...
	public function delete()
    {
        ...
        if(!$chk){
            ...
            if($this->master->delete('department_courses', $chk, 'course_id')){
