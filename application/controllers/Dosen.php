...
class Dosen extends CI_Controller
{
	...
	public function save()
	{
		...
		if ($method == 'add') {
			$u_nip = '|is_unique[instructors.employee_id]';
			$u_email = '|is_unique[instructors.email]';
		} else {
			$dbdata 	= $this->master->getDosenById($id_dosen);
			$u_nip		= $dbdata->nip === $nip ? "" : "|is_unique[instructors.employee_id]";
			$u_email	= $dbdata->email === $email ? "" : "|is_unique[instructors.email]";
		}
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$input = [
				'employee_id'	=> $nip,
				'full_name'		=> $nama_dosen,
				'email'			=> $email,
				'course_id'		=> $matkul
			];
			if ($method === 'add') {
				$action = $this->master->create('instructors', $input);
			} else if ($method === 'edit') {
				$action = $this->master->update('instructors', $input, 'id', $id_dosen);
			}
	...
	public function delete()
	{
		...
		if (!$chk) {
			...
			if ($this->master->delete('instructors', $chk, 'id')) {
	...
	public function do_import()
	{
		...
		$data = [];
		foreach ($input as $d) {
			// Accept both English and legacy Indonesian keys for safety
			$employee = isset($d->employee_id) ? $d->employee_id : (isset($d->nip) ? $d->nip : null);
			$fullname = isset($d->full_name) ? $d->full_name : (isset($d->nama_dosen) ? $d->nama_dosen : null);
			$course = isset($d->course_id) ? $d->course_id : (isset($d->matkul_id) ? $d->matkul_id : null);
			$data[] = [
		...
		$save = $this->master->create('instructors', $data, true);
		if ($save) {
			redirect('dosen');
		} else {
