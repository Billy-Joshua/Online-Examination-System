...
class Dashboard extends CI_Controller {
	...
	public function index()
	{
		...
		if ( $this->ion_auth->is_admin() ) {
			...
			$matkul = ['courses' => 'instructors.course_id = courses.id'];
			$data['dosen'] = $this->dashboard->get_where('dosen', 'nip', $user->username, $matkul)->row();

			$kelas = ['class_instructors' => 'class_instructors.class_id = classes.id'];
			$data['kelas'] = $this->dashboard->get_where('kelas_dosen', 'dosen_id' , $data['dosen']->id_dosen, $kelas, ['classes.name'=>'ASC'])->result();
		}else{
			$join = [
				'kelas b' 	=> 'a.kelas_id = b.id_kelas',
				'jurusan c'	=> 'b.jurusan_id = c.id'
			];
